...
def register():
    ...
    if not request.json.get('password', None):
        return jsonify({'msg': 'pass the password key'}), 400
...
def login():
    ...
    if not user:
        return jsonify({"msg": "Invalid Username Or Password"}), 401  # status code for authentication errors
...
def test():
    """
        Test Flask Api
    """
    return url_for('bucketlists.bucketlists')
